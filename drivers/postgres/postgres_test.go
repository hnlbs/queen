...
const PostgreSQLTestDSN = "postgresql://user:password@localhost:5432/mydatabase?sslmode=disable"
...
func setupTestDB(t *testing.T) (*sql.DB, func()) {
	...
	// Connect to PostgreSQL (using port 3307 to avoid conflicts)
	dsn := os.Getenv("POSTGRESQL_TEST_DSN")
	if dsn == "" {
		// Default DSN for local testing
		dsn = PostgreSQLTestDSN
	}
...
func TestIntegrationLocking(t *testing.T) {
	...
	db2, err := sql.Open("pgx", PostgreSQLTestDSN)
	if err != nil {
